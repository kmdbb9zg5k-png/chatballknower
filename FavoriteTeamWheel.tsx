...
export function FavoriteTeamWheel({value,onConfirm,onCancel}:Props){
 ...
 const move=(n:number)=>{if(!n)return;setIndex(i=>(i+n+NFL_TEAMS.length)%NFL_TEAMS.length);setPulse(p=>p+1);if('vibrate'in navigator)navigator.vibrate?.(8)};
 const visible=useMemo(()=>[-3,-2,-1,0,1,2,3].map(offset=>({offset,team:NFL_TEAMS[(index+offset+NFL_TEAMS.length)%NFL_TEAMS.length]})),[index]);
 useEffect(()=>()=>{if(timer.current)window.clearTimeout(timer.current)},[]);
 const randomSpin=()=>{
   if(spinning)return;
   setSpinning(true);
   let step=0;const total=18+Math.floor(Math.random()*15);const direction=Math.random()>.5?1:-1;
   const tick=()=>{move(direction);step++;if(step<total){const delay=38+Math.pow(step/total,3)*190;timer.current=window.setTimeout(tick,delay)}else setSpinning(false)};
   tick();
 };
 const tapNeighbor=(offset:number)=>{if(spinning)return;move(offset)};
 return <div className="fixed inset-0 z-[120] overflow-hidden bg-[#050607] text-white flex flex-col items-center justify-center px-3 sm:px-4 select-none" onTouchStart={e=>touch.current={x:e.touches[0].clientX,t:Date.now()}} onTouchEnd={e=>{if(!touch.current||spinning)return;const d=e.changedTouches[0].clientX-touch.current.x;const dt=Math.max(1,Date.now()-touch.current.t);const speed=Math.abs(d)/dt;if(Math.abs(d)>28){const amount=speed>.9?3:speed>.55?2:1;move((d<0?1:-1)*amount)}touch.current=null}}>
  ...
  <div className="relative z-10 w-full max-w-4xl h-[350px] sm:h-[465px] flex items-center justify-center [perspective:1250px]">
   ...
   {visible.map(({offset,team:t}:any)=>{const abs=Math.abs(offset);const center=offset===0;const x=offset*112;const z=-abs*145;const ry=-offset*23;const scale=center?1.33:abs===1?.88:abs===2?.65:.48;return <button key={t.code} disabled={spinning} onClick={()=>tapNeighbor(offset)} aria-label={`Select ${t.city} ${t.name}`} className="absolute transition-all duration-300 ease-out flex flex-col items-center justify-center touch-manipulation outline-none" style={{transform:`translateX(${x}px) translateZ(${z}px) rotateY(${ry}deg) scale(${scale})`,opacity:center?1:abs===1?.68:abs===2?.32:.12,zIndex:20-abs,filter:center?'drop-shadow(0 16px 24px rgba(0,0,0,.7))':`saturate(${abs===1?.7:.35}) brightness(${abs===1?.72:.48}) blur(${abs===3?1.5:0}px)`}}>
    <div className={`relative w-28 h-28 sm:w-36 sm:h-36 rounded-full grid place-items-center border transition-all ${center?'border-[#D4AF37] bg-[#111]/95 shadow-[0_0_0_5px_rgba(212,175,55,.08),0_0_38px_rgba(212,175,55,.30),inset_0_0_24px_rgba(255,255,255,.04)]':'border-white/10 bg-[#0b0d0f]/90'}`}>
     <img src={teamLogo(t.code)} alt={`${t.name} logo`} draggable={false} className={`${center?'w-[76%] h-[76%]':'w-[68%] h-[68%]'} object-contain drop-shadow-[0_8px_10px_rgba(0,0,0,.65)]`} onError={e=>{const el=e.currentTarget;el.style.display='none';const next=el.nextElementSibling as HTMLElement|null;if(next)next.style.display='block'}}/>
