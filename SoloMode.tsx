...
export const SoloMode:React.FC=()=>{
  ...
  return <div className="min-h-screen bg-[#090909] text-white px-4 sm:px-8 py-6"><div className="mx-auto max-w-7xl">
    ...
    {stage==='regular'&&<div className="space-y-4">
      ...
      <div className="grid grid-cols-3 gap-2"><SimpleStat label="RECORD" value={`${wins}-${losses}`}/><SimpleStat label="WEEK" value={`${weeks.length}/17`}/><SimpleStat label="TEAM OVR" value={`${ratings.overall}`}/></div>
      ...
      {active.length>0&&<div className="border border-red-500/25 bg-red-500/5 p-4"><div className="text-xs font-black text-red-300">INJURY REPORT</div>{active.map(i=><div key={i.playerId} className="text-sm mt-1"><b>{i.playerName}</b> • {i.weeks} game(s) left</div>)}</div>}
    ...
    {stage==='playoffs'&&<div className="max-w-5xl mx-auto"><section className="text-center border border-[#D4AF37]/25 bg-[#D4AF37]/5 p-6 mb-5"><div className="text-[10px] text-[#D4AF37] font-black tracking-[.2em]">STEP 3 — WIN FOUR GAMES</div><Trophy className="mx-auto text-[#D4AF37] mt-3" size={54}/><h3 className="text-4xl sm:text-5xl font-black mt-2">PLAYOFFS</h3><p className="text-zinc-400 mt-2">Lose once and the run is over.</p></section><div className="grid grid-cols-2 md:grid-cols-4 gap-2">{['WILD CARD','DIVISIONAL','CONFERENCE','SUPER BOWL'].map((r,i)=>{const x=playoffs[i];return <div key={r} className={`p-4 border ${round===r?'border-[#D4AF37] bg-[#D4AF37]/10':'border-white/10 bg-[#111]'}`}><div className="text-[9px] text-[#D4AF37] font-black">{r}</div>{x?<><b>{x.won?'WIN':'LOSS'} {x.you}-{x.them}</b><div className="text-xs text-zinc-500">{x.opponent}</div></>:<div className="text-zinc-600 mt-2">TBD</div>}</div>})}</div>{active.length>0&&<div className="mt-4 border border-red-500/20 p-4 text-red-300 text-sm">Injured: {active.map(i=>i.playerName).join(', ')}. Backups are automatically in.</div>}{round&&<button onClick={playRound} className="mt-5 w-full py-5 bg-[#D4AF37] text-black font-black text-xl">PLAY {round}</button>}</div>}

    {stage==='finished'&&<div className="max-w-6xl mx-auto"><div className={`text-center border p-7 ${champion?'border-[#D4AF37]/60 bg-[#D4AF37]/5':'border-white/10 bg-[#111]'}`}><Crown className={champion?'mx-auto text-[#D4AF37]':'mx-auto text-zinc-500'} size={64}/><div className="text-[#D4AF37] text-xs font-black tracking-[.3em] mt-3">SEASON COMPLETE</div><h3 className="text-4xl sm:text-6xl font-black">{champion?'SUPER BOWL CHAMPION':'YOUR RUN IS OVER'}</h3><p className="text-zinc-300 mt-3">{message}</p><div className="grid grid-cols-2 md:grid-cols-6 gap-2 mt-6"><SimpleStat label="RECORD" value={`${wins}-${losses}`}/><SimpleStat label="TEAM OVR" value={`${ratings.overall}`}/><SimpleStat label="CHEMISTRY" value={`${chemistry.score}`}/><SimpleStat label="DRAFT" value={grade.letter}/><SimpleStat label="BK SCORE" value={`${grade.score}`}/><SimpleStat label="BK RATING" value={`${careerRating}`}/></div></div>
      <div className="grid lg:grid-cols-2 gap-4 mt-5"><Panel title="SEASON AWARDS" icon={<Award size={18}/>}>{mvpRace[0]&&<div className="bg-[#D4AF37]/10 border border-[#D4AF37]/30 p-3 mb-2"><div className="text-[9px] text-[#D4AF37] font-black">SIMULATED LEAGUE MVP</div><b>{mvpRace[0].name}</b></div>}{awards.map(a=><div key={a.award} className="bg-[#181818] p-3 mb-2"><div className="text-[9px] text-[#D4AF37] font-black">{a.award}</div><b>{a.winner}</b></div>)}</Panel><Panel title="ACHIEVEMENTS" icon={<Trophy size={18}/>}>{achievementsForRun(wins,losses,champion,grade.score,roster).map(a=><div key={a} className="py-2 border-b border-white/5">🏆 {a}</div>)}</Panel></div>
      <details className="mt-4 bg-[#111] border border-white/10"><summary className="cursor-pointer p-4 font-black">FULL SEASON DETAILS</summary><div className="p-4 pt-0 space-y-4"><Panel title="FULL GAME LOG" icon={<Activity size={18}/>}><WeekLog weeks={weeks}/>{playoffs.map(x=><div key={x.round} className="flex justify-between py-2 border-b border-white/5"><span>{x.won?'W':'L'} • {x.round} vs {x.opponent}</span><b>{x.you}-{x.them}</b></div>)}</Panel><Panel title="FULL SEASON STATS" icon={<BarChart3 size={18}/>}><SeasonStats rows={seasonStats}/></Panel><Panel title="INJURY HISTORY" icon={<ShieldAlert size={18}/>}>{injuryHistory.length?injuryHistory.map(i=><div key={`${i.playerId}-${i.week}`} className="py-2 border-b border-white/5"><b>{i.playerName}</b><div className="text-xs text-zinc-500">Week {i.week} • {i.severity}</div></div>):<p className="text-zinc-500 text-sm">No injuries.</p>}</Panel>{storylines.length>0&&<Panel title="YOUR SEASON IN HEADLINES" icon={<Flame size={18}/>}><div className="grid md:grid-cols-2 gap-2">{storylines.map(s=><StoryMini key={s.id} story={s}/>)}</div></Panel>}</div></details>
...
const Roster=({title,players,target,onRemove}:{title:string,players:Player[],target:number,onRemove:(p:Player)=>void})=><div className="bg-[#111] border border-white/10 p-3"><div className="flex justify-between text-[10px] font-black text-[#D4AF37] mb-2"><span>{title}</span><span>{players.length}/{target}</span></div>{players.length===0?<div className="text-xs text-zinc-600 py-3">No players yet.</div>:<div className="space-y-1 max-h-[260px] overflow-y-auto">{players.map(p=><div key={p.id} className="flex justify-between items-center bg-[#181818] px-3 py-2"><span className="truncate text-sm"><b>{p.position}</b> {p.name} <small className="text-zinc-600">{p.ovr}</small></span><button onClick={()=>onRemove(p)} className="p-1"><Trash2 size={15}/></button></div>)}</div>}</div>;
...
const StoryCard=({story}:{story:Storyline})=><div className={`border p-4 ${story.tone==='red'?'border-red-500/30 bg-red-500/5':story.tone==='green'?'border-green-500/30 bg-green-500/5':story.tone==='blue'?'border-cyan-500/30 bg-cyan-500/5':'border-[#D4AF37]/40 bg-[#D4AF37]/5'}`}><div className="text-[9px] font-black tracking-[.2em] text-[#D4AF37]">{story.tag} • WEEK {story.week}</div><h3 className="text-xl sm:text-2xl font-black mt-1">{story.headline}</h3><p className="text-sm text-zinc-400 mt-1">{story.deck}</p></div>;
...
const TradeCard=({offer,onAccept,onDecline}:{offer:SoloTradeOffer;onAccept:()=>void;onDecline:()=>void})=><div className="border border-cyan-400/30 bg-cyan-400/5 p-4"><div className="flex items-center gap-2 text-cyan-300 text-xs font-black"><ArrowRightLeft size={16}/>TRADE OFFER</div><p className="text-sm text-zinc-400 mt-2">{offer.reason}</p><div className="grid grid-cols-[1fr_auto_1fr] items-center gap-3 mt-3"><div className="bg-[#111] p-3"><div className="text-[9px] text-zinc-600">YOU SEND</div><b>{offer.outgoing.name}</b><div className="text-xs text-zinc-500">{offer.outgoing.position} • {offer.outgoing.ovr}</div></div><ArrowRightLeft/><div className="bg-[#111] p-3"><div className="text-[9px] text-cyan-300">YOU GET</div><b>{offer.incoming.name}</b><div className="text-xs text-zinc-500">{offer.incoming.position} • {offer.incoming.ovr}</div></div></div><div className="grid grid-cols-2 gap-2 mt-3"><button onClick={onDecline} className="py-3 border border-white/10 font-black">DECLINE</button><button onClick={onAccept} className="py-3 bg-cyan-300 text-black font-black">ACCEPT</button></div></div>;
const LeaderList=({rows}:{rows:any[]})=><div>{rows.map(x=><div key={x.category} className="grid grid-cols-[80px_1fr_auto] gap-2 py-2 border-b border-white/5 text-sm"><span className="text-[9px] font-black text-zinc-500">{x.category}</span><span className={x.isUser?'text-[#D4AF37] font-black':''}>{x.name} <small className="text-zinc-600">{x.team}</small></span><b>{x.value}</b></div>)}</div>;
const MvpList=({rows}:{rows:any[]})=><div>{rows.map((x,i)=><div key={`${x.name}-${i}`} className={`flex justify-between gap-2 py-2 border-b border-white/5 text-sm ${x.isUser?'text-[#D4AF37]':''}`}><span>#{i+1} {x.name} <small className="text-zinc-600">{x.position}</small></span><b>{Number(x.score).toFixed(1)}</b></div>)}</div>;
const SeasonStats=({rows}:{rows:any[]})=><div className="overflow-x-auto"><table className="w-full text-xs min-w-[720px]"><thead className="text-zinc-600"><tr><th className="text-left py-2">PLAYER</th><th>POS</th><th>PASS</th><th>PTD</th><th>RUSH</th><th>RTD</th><th>REC</th><th>RECYD</th><th>SACK</th><th>INT</th></tr></thead><tbody>{rows.map(x=><tr key={x.playerId} className="border-t border-white/5"><td className="py-2 font-black">{x.name}</td><td className="text-center">{x.position}</td><td className="text-center">{x.passYds||0}</td><td className="text-center">{x.passTD||0}</td><td className="text-center">{x.rushYds||0}</td><td className="text-center">{x.rushTD||0}</td><td className="text-center">{x.receptions||0}</td><td className="text-center">{x.recYds||0}</td><td className="text-center">{x.sacks||0}</td><td className="text-center">{x.picks||0}</td></tr>)}</tbody></table></div>;
