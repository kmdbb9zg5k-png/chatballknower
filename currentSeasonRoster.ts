 ...
 *
 * Bundled overrides are only a fallback. The live app now applies the verified
 * 2026 roster map in main.tsx using name + position, which safely handles
 * duplicate names such as Justin Jefferson (MIN WR / CLE LB).
 */
...
export function applyCurrent2026Roster(rawPlayers: Player[]): Player[] {
  ...
  const corrected = seedPlayers
    ...
    .map((player): Player => {
      // Keep this fallback deliberately distinct from Ball Knower Live's old
      // name-only injection signature. main.tsx owns live team synchronization.
