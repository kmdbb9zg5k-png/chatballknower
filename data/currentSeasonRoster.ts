 ...
 *
 * Ball Knower Live injects nflverse's daily 2026 name->team map into the
 * `nextTeam` line below. A live match updates the player's team. If a feed
 * lookup misses because of a naming mismatch, we keep the player available
 * instead of deleting the entire draft pool. Explicitly inactive/retired
 * players are removed below.
 */
export const CURRENT_2026_QB_STARTERS: Record<string, string> = {
  ...
  ARI: 'Jacoby Brissett', LAR: 'Matthew Stafford', SF: 'Brock Purdy', SEA: 'Sam Darnold',
};

export const MADDEN_27_QB_OVERRIDES: Record<string, number> = {
  'Josh Allen': 99, 'Matthew Stafford': 99, 'Joe Burrow': 97, 'Lamar Jackson': 94,
  'Patrick Mahomes': 93, 'Drake Maye': 92, 'Dak Prescott': 91, 'Caleb Williams': 90,
  'Justin Herbert': 90, 'Jared Goff': 88, 'Sam Darnold': 87, 'Jordan Love': 86,
  'Brock Purdy': 85, 'Trevor Lawrence': 84, 'Baker Mayfield': 83, 'Jalen Hurts': 82,
  'Bo Nix': 81, 'Jayden Daniels': 80, 'Aaron Rodgers': 79, 'Bryce Young': 78,
  'Daniel Jones': 78, 'Jaxson Dart': 77, 'Tyler Shough': 77, 'C.J. Stroud': 76,
  'Cam Ward': 75, 'Kyler Murray': 75, 'Fernando Mendoza': 74, 'Malik Willis': 74,
  'Tua Tagovailoa': 74, 'Kirk Cousins': 73, 'Michael Penix Jr.': 73, 'Geno Smith': 72,
  'Jacoby Brissett': 72, 'Deshaun Watson': 69, 'Shedeur Sanders': 69,
};

const TEAM_OVERRIDES: Record<string, string> = {
  'Malik Willis': 'MIA', 'Geno Smith': 'NYJ', 'Aaron Rodgers': 'PIT', 'Daniel Jones': 'IND',
  'Kirk Cousins': 'LV', 'Kyler Murray': 'MIN', 'Jacoby Brissett': 'ARI', 'Sam Darnold': 'SEA',
  'Tua Tagovailoa': 'ATL', 'Davante Adams': 'LAR', 'A.J. Brown': 'NE',
  'Jaylen Waddle': 'DEN', 'Myles Garrett': 'LAC',
};

// Known players who must never return to the active Ball Knower draft pool.
// The live roster feed still handles current team changes; this set is the
// safety net for retired/inactive legacy records that may exist in old files.
const INACTIVE_2026 = new Set([
  'Zack Martin',
  'Russell Wilson',
]);
...
export function applyCurrent2026Roster(rawPlayers: Player[]): Player[] {
  ...
  const corrected = seedPlayers
    .filter(player => !INACTIVE_2026.has(player.name))
    .map((player): Player => {
